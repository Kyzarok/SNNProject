'''
Module defining the `SpikeSource` class (which is only used for documentation
purposed.
'''
__all__ = ['SpikeSource']
...
class SpikeSource(object):
    ...
    * A `clock` attribute, this will be used as the default clock for objects
    ...
    .. attribute:: spikes
        ...
        An array of ints, each from 0 to ``len(obj)-1`` with no repeats (and in
        sorted order). Updated each time step.
